@extends('candidate.layouts.main')
@push('styles')
@endpush
@section('content')
@php
    use App\Models\Resume;
@endphp
<div class="content-full">
        <div class="mt-3">
            @include('candidate.includes.message')
            <h2 style="display: inline-block;">Applying for a job</h2> @if ($isEmpty) <span class="text-danger"> (Applied for a job)</span> @endif
            <div class="resume">
                <div class="personal-content mb-5">
                    <table>
                        <tbody>
                            <tr>
                                <th><span>Job name</span></th>
                                <td>{{ $job->job_title }}</td>
                            </tr>
                            <tr>
                                <th><span>Company name</span></th>
                                <td>{{ $job->company->name }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="personal mt-3">
                    <div class="personal-header">
                        <img class="icon-header" src="{{ asset('images/icon-user.svg') }}" height="18" width="18" alt="">
                        Personal information
                        <a href="{{ route('candidate.resume.edit', ['id' => $resume->id]) }}" class="btn btn-dark">
                            <img src="{{ asset('images/icon-edit.svg') }}" height="12" width="12" alt="">Edit
                        </a>
                    </div>
                    <div class="personal-content">
                        <table>
                            <tbody>
                                <tr>
                                    <th><span>Full name</span></th>
                                    <td>{{ auth('web')->user()->full_name }}</td>
                                </tr>
                                <tr>
                                    <th><span>Email</span></th>
                                    <td>{{ auth('web')->user()->email }}</td>
                                </tr>
                                <tr>
                                    <th><span>Age</span></th>
                                    <td>{{ $resume->age }}</td>
                                </tr>
                                <tr>
                                    <th><span>Phone</span></th>
                                    <td>{{ $resume->phone }}</td>
                                </tr>
                                <tr>
                                    <th><span>Country</span></th>
                                    <td>{!! nl2br(e($resume->country)) !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Current residence</span></th>
                                    <td>{!! nl2br(e($resume->address)) !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Facebook</span></th>
                                    <td>{{ $resume->facebook }}</td>
                                </tr>
                                <tr>
                                    <th><span>Hobby</span></th>
                                    <td>{!! nl2br(e($resume->hobby)) !!}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="personal mt-3">
                    <div class="personal-header">
                        <img class="icon-header" src="{{ asset('images/icon-user.svg') }}" height="18" width="18" alt="">
                        Experience
                        <a href="{{ route('candidate.resume.experience', ['id' => $resume->id]) }}" class="btn btn-dark">
                            <img src="{{ asset('images/icon-edit.svg') }}" height="12" width="12" alt="">Edit
                        </a>
                    </div>
                    <div class="personal-content">
                        <table>
                            <tbody>
                                <tr>
                                    <th><span>Certificate</span></th>
                                    <td>{!! $resume->certificate !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Skill</span></th>
                                    <td>{!! $resume->skill !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Industries</span></th>
                                    <td>{!! $resume->industry_labels !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Occupations</span></th>
                                    <td>{!! $resume->occupation_labels !!}</td>
                                </tr>
                                <tr>
                                    <th><span>Current salary</span></th>
                                    <td>{{ $resume->current_salary }} USD</td>
                                </tr>
                                <tr>
                                    <th><span>Curriculum vitae</span></th>
                                    <td>{{ optional($resume->candidate->attachment)->upload_file_name }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>

    <form action="{{ route('candidate.job.apply', $job->id) }}" method="post">
        @csrf
        <div class="j-apply text-center mt-5">
            <button type="submit" class="j-app {{ $isEmpty ? 'disabled' : '' }}" @if ($isEmpty) disabled @endif>
                <img src="{{ asset('images/icon-answer-white.svg') }}" alt="" class="pr-1">
                Apply for</button>
        </div>
    </form>

</div>
@endsection

@push('scripts')
    <script src="{{ mix('js/app.js') }}"></script>
@endpush
